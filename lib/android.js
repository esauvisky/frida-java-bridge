...
function _getApi () {
  ...
  if (isArt) {
    ...
    artController = makeArtController(vm, temporaryApi);
...
function makeArtController (vm, api) {
...
function instrumentArtFixupStaticTrampolines () {
  const fixupImpl = Module.findExportByName('libart.so', '_ZN3art11ClassLinker22FixupStaticTrampolinesEPNS_6ThreadENS_6ObjPtrINS_6mirror5ClassEEE');
  if (fixupImpl === null)
    return;

  Interceptor.attach(fixupImpl, {
    onLeave() {
      console.log('Applying fixups');
      const api = getApi();
      const { artNterpEntryPoint, artQuickToInterpreterBridge } = api;
      artController.replacedMethods.onFixupsApplied(getArtMethodSpec(api.vm).offset.quickCode, artNterpEntryPoint, artQuickToInterpreterBridge);
    }
  });
}
